    ...
    <div class="items-grid" v-loading="loading">
      ...
      >
        <div class="item-image" v-if="item.image">
          <el-image :src="getImageUrl(item.image)" fit="cover" />
        </div>
...
const fetchItems = async () => {
  loading.value = true
  try {
    const res = await api.get('/receiver/items', {
      params: {
...
const fetchCategories = async () => {
  try {
    const res = await api.get('/receiver/categories')
...
const submitReceive = async () => {
  ...
  try {
    await api.post('/receiver/receive', {
