    ...
    <div class="filter-bar">
      <el-form :inline="true" :model="filters">
        ...
        <el-form-item label="入库类型">
          <el-select v-model="filters.inbound_type" placeholder="全部" clearable>
            <el-option v-for="(label, value) in inboundTypes" :key="value" :label="label" :value="value" />
          </el-select>
        </el-form-item>
        ...
        <el-form-item>
          ...
          <el-button type="success" @click="handleAdd" v-if="authStore.hasAnyRole(['admin', 'operator'])">新增入库</el-button>
      ...
      <el-form ref="formRef" :model="form" :rules="rules" label-width="100px" :disabled="isView">
        <el-form-item label="物品" prop="item_id" v-if="!isView">
          <el-select v-model="form.item_id" placeholder="请选择物品" filterable style="width: 100%" @change="handleItemChange">
            <el-option v-for="item in itemList" :key="item.id" :label="`${item.item_code} - ${item.name}`" :value="item.id" />
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="入库类型" prop="inbound_type">
          <el-select v-model="form.inbound_type" placeholder="请选择入库类型" style="width: 100%">
            <el-option v-for="(label, value) in inboundTypes" :key="value" :label="label" :value="value" />
          </el-select>
        </el-form-item>
        <el-row :gutter="20">
      ...
      >
        <template #sub-title>
          <div style="text-align: left">
            ...
            <p v-if="importResult.new_item_count > 0">
              新建物品: <span style="color: #409eff; font-weight: bold">{{ importResult.new_item_count }}</span> 个
            </p>
...
const dialogTitle = computed(() => isView.value ? '查看入库记录' : '新增入库')
...
const fetchInboundTypes = async () => {
  try {
    const res = await api.get('/inbound/types')
    inboundTypes.value = res
  } catch (error) {
    console.error(error)
  }
}
...
const fetchItemList = async () => {
  try {
    const res = await api.get('/items', { params: { per_page: 1000 } })
...
const handleImport = async () => {
  ...
  try {
    ...
    if (res.ok) {
      ...
      importResult.new_item_count = data.new_item_count
